...
- **Who can deploy**: the Netlify CLI is logged in **only on the local Windows machine**. A Claude Code web/cloud session has no CLI, no `.env.production` (gitignored, 0 commits) and no network route to `api.netlify.com` (the sandbox returns `CONNECT tunnel failed, 403`) — so it cannot deploy and must not imply otherwise. The durable fix is linking the Netlify site to the Git repo (see the deploy-live skill): the site already has all four `VITE_*` vars but its repository reads **Not linked** as of 2026-09, which is why merges to `main` deploy nothing.
- **Deploy (2026-09)**: `.github/workflows/deploy.yml` builds + deploys to Netlify on every push to `main` touching `mass-market-app/**` (or via workflow_dispatch). Needs **exactly one** repo secret — `NETLIFY_AUTH_TOKEN`; the other five build inputs are public and inlined as defaults in the workflow's `env:` block (`VITE_API_URL`, `VITE_SUPABASE_URL`, `NETLIFY_SITE_ID`, `VITE_DEV_AUTH=false`, and `VITE_SUPABASE_ANON_KEY` — which is public by design and already committed verbatim in `src/lib/supabase.js`), each overridable by a repo secret of the same name. Until the token exists the job **skips and stays green** with the instructions in the run summary — it never fails a merge and never ships a half-configured build. It guards the artifact before shipping (all prerendered pages present, no ngrok **domain**, bundle really contains `VITE_API_URL`, and `dist/_redirects` still carries `/go/*` so affiliate attribution cannot be dropped silently) and verifies the live entry-bundle hash matches the build. NB: a bare `grep ngrok` over the bundle false-positives — every build contains the legacy `ngrok-skip-browser-warning` header. A GitHub runner can reach `api.mocaintel.com`, so the ~160 prerendered destination pages are included when the tunnel is up. The manual CLI path below still works and remains the fallback.
