...
- **The scraper already auto-captures for this carrier (column "AUTO" below):** the plan
  ...
  — see AGENTS.md "After Code Changes". No code edit needed.
...
3. If you edited PlanCard.jsx (the map): `cd mass-market-app && npm run build`, then the
   user drag-deploys `dist/` to Netlify (manual — see AGENTS.md "Deployment"). The DB/API
...
> **Skills don't run themselves.** This runbook only fires when Codex is in the loop. The
