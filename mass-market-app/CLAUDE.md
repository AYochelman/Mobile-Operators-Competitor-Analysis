...
| pages/CockpitPage.jsx | **Executive situation room** (`/cockpit`, nav group ניטור, feature flag `hide_cockpit`). One screen for a סמנכ"ל: verdict bar (our rank on the GB-weighted ₪/GB ladder vs market median), 5 KPI tiles, market-index trend (ours vs market median from `/api/history/daily` — days with <3 reporting carriers are dropped, `carrier='*'` aggregate rows ignored), impact-ranked material moves (`/api/market-movers`, >5%), single-hue positioning ladder, threat / opportunity / attention signals, the cached domestic AI narrative + a data-freshness footer (36h domestic/abroad, 72h global). Reads only existing endpoints; every source degrades independently. The whole data layer is `hooks/useCockpitData.js` — PURE exported derivations (`blendedPpgb`, `buildLadder`, `buildActivity`, `buildTrend`, `buildSignals`) + the `useCockpitData(oursCarrier, days)` hook, smoke-testable in node like `useDashboardPlans.js`. `blendedPpgb` mirrors `compute_executive_metrics` (SUM(price)/SUM(GB), unlimited excluded). Chart colours were run through the dataviz palette validator: the mocha family is one hue, so the trend is ONE coloured series (ours) + a dashed neutral market reference, and the ladder is single-hue with only "ours" tinted `#c9622f` — no rank-coloured ramps. |
...
| hooks/useCockpitData.js | Data layer for `/cockpit` (see CockpitPage above): pure derivations + fetching hook over `/api/plans`, `/api/changes`, `/api/abroad-*`, `/api/global-plans`, `/api/market-movers`, `/api/history/daily` (new `api.getHistoryDaily`), `/api/executive-summary` |
