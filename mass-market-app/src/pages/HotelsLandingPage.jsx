...
const COPY = {
  he: {
    ...
    // Hard break after "ושואל" keeps the quoted question "איך מתחברים?" on its
    // own line; the ⁠ (word joiner) glues the closing ״ to "?" so it never wraps
    // off alone if that line ever breaks.
    heroLine1: 'האורח נוחת ושואל',
    heroLine1b: '״איך מתחברים?⁠״',
    heroLine2pre: 'המלון שלכם עונה - ',
    heroEm: 'ומרוויח',
    heroLine2post: '.',
    heroLead: 'פורטל ממותג בצבעי המלון שמשווה לאורח, בזמן אמת, את כל חבילות ה-eSIM והסים לתיירים בישראל - למעלה מ-30 ספקים גלובליים ו-10 מפעילים מקומיים, מתעדכן פעמיים ביום.',
    heroLead2: 'בלי מלאי, בלי תפעול - ועם עמלה למלון על כל רכישה.',
    heroCtaDemo: '▶ צפו בהדגמה החיה',
    heroCtaContact: 'תיאום שיחת היכרות',
    stats: [
      { pre: '', b: '30+', post: ' ספקי eSIM גלובליים' },
    ...
    faqs: [
      ...
      { q: 'המחירים באמת מתעדכנים? מי עומד מאחורי זה?', a: 'כן. Guest Connect רץ על MOCA - מערכת מודיעין תחרותי שסורקת את כל שוק הסלולר הישראלי ולמעלה מ-30 ספקי eSIM גלובליים פעמיים ביום.' },
    ],
  ...
  en: {
    ...
    heroEm: 'and earns',
    heroLine2post: '.',
    heroLead: 'A portal branded in your hotel’s colors that shows guests, in real time, every eSIM and tourist SIM deal in Israel - over 30 global providers and 10 local operators, refreshed twice a day.',
    ...
    stats: [
      { pre: '', b: '30+', post: ' global eSIM providers' },
    ...
    faqs: [
      ...
      { q: 'Do prices really update? Who’s behind this?', a: 'Yes. Guest Connect runs on MOCA - a competitive-intelligence system that scans the entire Israeli mobile market and over 30 global eSIM providers twice a day.' },
    ],
...
#hl-app .hero h1{font-family:'Assistant',system-ui,sans-serif;font-weight:900;color:var(--dark);font-size:clamp(30px,5vw,54px);line-height:1.15;max-width:26ch}
  ...
  return (
    <div id="hl-app" dir={t.dir} lang={lang}>
      ...
      <div className="topbar">
        <div className="wrap">
          <a className="logo" href="#top"><span className="bolt"><BoltMark size={19} /></span><span>MOCA<small>GUEST CONNECT</small></span></a>
          <nav>
      ...
      <header className="hero" id="top">
        <div className="wrap">
          ...
          <h1>{t.heroLine1}{t.heroLine1b && <><br />{t.heroLine1b}</>}<br />{t.heroLine2pre}<em>{t.heroEm}</em>{t.heroLine2post}</h1>
          <p className="lead">{t.heroLead}<br />{t.heroLead2}</p>
