...
export default function ChangePasswordModal({ open, onClose }) {
  ...
  const submit = async (e) => {
    ...
    if (next === current)  { setError('הסיסמה החדשה זהה לנוכחית'); return }
    setBusy(true)
    try {
  ...
  return (
    <Modal open={open} onClose={close} title="שינוי סיסמה" maxWidth="max-w-sm">
      {done ? (
        <div className="text-center py-4 space-y-4">
          <div className="text-3xl text-moca-down">✓</div>
          <p className="text-sm text-moca-text">הסיסמה עודכנה בהצלחה.</p>
          <Button size="sm" onClick={close}>סגור</Button>
        </div>
      ) : (
        <form onSubmit={submit} className="space-y-3">
          <div>
            <label className="block text-xs text-moca-sub mb-1 text-right">סיסמה נוכחית</label>
          ...
          <div>
            <label className="block text-xs text-moca-sub mb-1 text-right">סיסמה חדשה</label>
            <input type="password" value={next} onChange={e => setNext(e.target.value)} className={inputCls} dir="ltr" autoComplete="new-password" placeholder="לפחות 6 תווים" required />
          </div>
          <div>
            <label className="block text-xs text-moca-sub mb-1 text-right">אימות סיסמה חדשה</label>
          ...
          <div className="flex gap-2 justify-start pt-1">
            <Button type="submit" size="sm" disabled={busy}>{busy ? 'מעדכן...' : 'עדכן סיסמה'}</Button>
            <Button type="button" size="sm" variant="secondary" onClick={close}>ביטול</Button>
          </div>
        </form>
      )}
    </Modal>
