...
- פורמט: כותרות מודגשות (## כותרת), נקודות קצרות תחת כל כותרת.`
    try {
      ...
      setError(e.message || tt('שגיאה בטעינת הסיכום', 'Failed to load the summary'))
    } finally {
      setLoading(false)
    }
  }, [carrierName, tt])
    ...
    <div className="text-right">
      {loading && (
        <div className="flex flex-col items-center py-12 gap-3">
          ...
          <p className="text-xs text-gray-500">{tt('Claude מנתח את הנתונים...', 'Claude is analyzing the data...')}</p>
        </div>
      )}
      ...
      {error && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 text-sm text-red-700">
          {error}
          <button onClick={loadInsights} className="block mt-2 text-xs text-red-600 underline">{tt('נסה שוב', 'Try again')}</button>
        </div>
      )}
      ...
      {!loading && !error && answer && (
        <div className="text-sm text-gray-700 leading-relaxed whitespace-pre-line">
          ...
          <div className="mt-4 pt-3 border-t border-gray-100 flex items-center justify-between">
            ...
            <span className="text-[10px] text-gray-400">{tt('נוצר על ידי Claude', 'Generated by Claude')} · {new Date().toLocaleString('he-IL')}</span>
          </div>
        </div>
      )}
    </div>
  ...
  const Trigger = (
    <button
      onClick={handleClick}
      title={tt(`דוח AI תחרותי על ${carrierName}`, `Competitive AI report on ${carrierName}`)}
    ...
    >
      ...
      {inline && open
        ? tt(`הסתר דוח על ${carrierName}`, `Hide report on ${carrierName}`)
        : tt(`דוח AI על ${carrierName}`, `AI report on ${carrierName}`)}
    </button>
  )
  ...
  return (
    <>
      {Trigger}
      <Modal open={open} onClose={() => setOpen(false)} title={tt(`דוח תחרותי — ${carrierName}`, `Competitive report — ${carrierName}`)} maxWidth="max-w-2xl">
        {Body}
      </Modal>
    </>
  )
