...
export default function MobileMoreSheet({ open, onClose, sections, title }) {
  const { tt } = useLang()
  const location = useLocation()
  const sheetTitle = title != null ? title : tt('תפריט', 'Menu')
    ...
    <>
      ...
      >
        <div className="sticky top-0 bg-white border-b border-moca-border/60 px-4 py-3 flex items-center justify-between">
          <span className="text-sm font-semibold text-moca-text">{sheetTitle}</span>
          <button
            onClick={onClose}
